...
<!-- private Integer id;
	...
	<div style="width: 400px;">
	 <form class="form form-horizontal" id="form-admin-add" action="${ctx}/fdy/exUpdate" method="post">
	...
	<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>负责班级：</label>
			<div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				<select name="bjId" class="select">
				    <c:forEach items="${bjs}" var="data" varStatus="l">
				    <option value="${data.id}" ${data.id == obj.bjId ? "selected" : ""}>${data.name}</option>
				    </c:forEach>
				</select>
				</span> 
			</div>
